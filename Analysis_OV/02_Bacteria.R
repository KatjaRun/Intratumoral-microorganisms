...
path_bacteria <- paste0(pathout, "/bacteria.rds")
path_plots <- paste0(pathout, "/Plots")


##### [1] Load Phyloseq object #####
bacteria <- readRDS(path_bacteria)

## Creating a table with easy accessible species names
taxdata <- as.data.frame(tax_table(bacteria))
...
bacteria_decontam <- subset_taxa(bacteria, Genus != "")

## Filtering out Singletons (taxa with only one read)
if (sum(taxa_sums(bacteria_decontam) == 1) == 0) {
  print("No taxa with count equal to 1. Skipping prune_taxa.")
} else {
  cat("Pruning taxa with count equal to 1: ", sum(taxa_sums(bacteria_decontam) == 1))
  bacteria_decontam <- prune_taxa(taxa_sums(bacteria_decontam) >1, bacteria_decontam)
}

# Calculating how many taxa were lost
cat("Taxa lost after singleton and unclassified genera removal:", 
    1 - ntaxa(bacteria_decontam)/ntaxa(bacteria))
...
sampledata <- as.data.frame(sample_data(bacteria_decontam))
...
bacteria_PlateCenterSubset <- subset_samples(bacteria_decontam, sample_names(bacteria_decontam) %in% 
                                               sampledata_PlateCenterSubset$sample_id)

cat("Insufficient samples:", nsamples(bacteria_decontam)-nsamples(bacteria_PlateCenterSubset))

# Applying decontam to filtered Phyloseq
decontam_bacteria <- isContaminant(seqtab = bacteria_PlateCenterSubset,
...
table(decontam_bacteria$contaminant)

# Pruning taxa based on the identified contaminants
bacteria_decontam <- prune_taxa(!decontam_bacteria$contaminant, bacteria_decontam)

# Calculating how much data was lost since the beginning
cat("Lost taxa after decontam by Plate-Center: ", 1 - sum(taxa_sums(bacteria_decontam)/sum(taxa_sums(bacteria))))

## Saving table of removed taxa
taxa_contam <- decontam_bacteria[decontam_bacteria$contaminant == "TRUE", ]
taxa_contam$tax_id <- row.names(taxa_contam)
taxa_contam <- merge(species_names, taxa_contam, by = "tax_id")

write.csv(taxa_contam, file = paste(pathout, "/removed_contam_bacteria.csv", sep = ""), row.names = F)

##### Saving the decontaminated output #####
saveRDS(bacteria_decontam, file = paste(pathout, "/bacteria_decontam.rds", sep = ""))

cat("Number of taxa at beginning: ", ntaxa(bacteria))
cat("Number of taxa after decontamination: ", ntaxa(bacteria_decontam))
cat("Number of taxa lost: ", ntaxa(bacteria) - ntaxa(bacteria_decontam))
...
diversity <- microbiome::diversity(bacteria_decontam, index = "Shannon")
diversity2 <- microbiome::diversity(bacteria_decontam, index = "inverse_simpson")
diversity <- merge(diversity, diversity2, by = "row.names")
richness <- microbiome::richness(bacteria_decontam)
rarity <- microbiome::rarity(bacteria_decontam)
dominance <- microbiome::dominance(bacteria_decontam)

# Combining all data frames and adding a constant x-axis for plotting
bacteria_alpha <- cbind(diversity, richness, rarity, dominance)
bacteria_alpha$constant_x <- 0

## Creating ViolinPLot for observed species richness
observed <- ggplot(bacteria_alpha, aes(x = constant_x, y = observed)) +
...
chao1 <- ggplot(bacteria_alpha, aes(x = constant_x, y = chao1)) +
  geom_violin(trim = FALSE, fill = "skyblue") +
  geom_boxplot(width = 0.05, fill = "white") +
  # geom_jitter(color = "midnightblue", alpha = 0.5) +
  theme_light() +
...
shannon <- ggplot(bacteria_alpha, aes(x = constant_x, y = shannon)) +
  geom_violin(trim = FALSE, fill = "skyblue") +
  geom_boxplot(width = 0.05, fill = "white") +
  # geom_jitter(color = "midnightblue", alpha = 0.5) +
  theme_light() +
...
inv_simpson <- ggplot(bacteria_alpha, aes(x = constant_x, y = inverse_simpson)) +
  geom_violin(trim = FALSE, fill = "skyblue") +
  geom_boxplot(width = 0.05, fill = "white") +
  # geom_jitter(color = "midnightblue", alpha = 0.5) +
  theme_light() +
...
dominance_core <- ggplot(bacteria_alpha, aes(x = constant_x, y = core_abundance * 100)) +
  geom_violin(trim = FALSE, fill = "skyblue") +
  geom_boxplot(width = 0.05, fill = "white") +
  # geom_jitter(color = "midnightblue", alpha = 0.5) +
  theme_light() +
...
low_abundance <- ggplot(bacteria_alpha, aes(x = constant_x, y = low_abundance * 100)) +
  geom_violin(trim = FALSE, fill = "skyblue") +
  geom_boxplot(width = 0.05, fill = "white") +
  # geom_jitter(color = "midnightblue", alpha = 0.5) +
  theme_light() +
...
ggsave(paste(path_plots, "/Bacteria_alpha.png", sep = ""),
...
ord_NMDs <- phyloseq::ordinate(bacteria_decontam, "NMDS", "bray", maxit=100000)

options(repr.plot.width = 3, repr.plot.height = 3)
plot_ordination(bacteria_decontam, ord_NMDs, type = "samples", color = "Subtype_mRNA", shape = "sample_type",
                title = "Bacteria: NMDs of samples") +
  theme_light() +
  theme(plot.margin = margin(1, 1, 1, 1, "line")) +
  coord_fixed(ratio = 1)

ggsave(paste0(path_plots, "/Bacteria_NMDS.png"), height = 10, width = 15, unit = "cm")

### Ordination using PCoA and bray distance
ord_PCOA <- ordinate(bacteria_decontam, "PCoA")

options(repr.plot.width = 3, repr.plot.height = 3)
plot_ordination(bacteria_decontam, ord_PCOA, type = "samples", color = "Subtype_mRNA", shape = "sample_type",
                title = "Bacteria: PCoA of samples") +
  theme_light() +
  theme(plot.margin = margin(1, 1, 1, 1, "line")) +
  coord_fixed(ratio = 1)

ggsave(paste0(path_plots, "/Bacteria_PCoA.png"), height = 10, width = 15, unit = "cm")
...
bacteria_rel = microbiome::transform(bacteria_decontam, "compositional")

## Plot for top 20 phyla
# Performing tax_glom at phylum level and saving 
bacteria_phy = speedyseq::tax_glom(bacteria_rel, taxrank = "Phylum")
saveRDS(bacteria_phy, paste0(pathout, "/bacteria_phyla.rds"))

# Sorting to get most abundant phyla and pruning the phyloseq
top20_phy = names(sort(taxa_sums(bacteria_phy), decreasing = TRUE))[1:20]
bacteria_top20_phy = prune_taxa(top20_phy, bacteria_phy)

# Checking relative abundance of phyla
cat("Mean abundance of phyla: ", mean(colSums(otu_table(bacteria_top20_phy))))
cat("Ranging from: ", min(colSums(otu_table(bacteria_top20_phy))), "to", 
    max(colSums(otu_table(bacteria_top20_phy))))

# Preparing data frame for plotting
top_phyla_OTU <- as.data.frame(otu_table(bacteria_top20_phy))
top_phyla_tax <- as.data.frame(tax_table(bacteria_top20_phy))
...
ggsave(paste0(path_plots, "/Bacteria_top_phyla.png"))  


## Plot for top 20 genera
# Performing tax_glom at genus level and saving
bacteria_gen = speedyseq::tax_glom(bacteria_rel, taxrank = "Genus")
saveRDS(bacteria_gen, paste0(pathout, "/bacteria_genera.rds"))

# Sorting to get most abundant phyla and pruning the phyloseq
top20_gen = names(sort(taxa_sums(bacteria_gen), decreasing = TRUE))[1:20]
bacteria_top20_gen = prune_taxa(top20_gen, bacteria_gen)

# Checking the relative abundance per sample
cat("Mean abundance of genera: ", mean(colSums(otu_table(bacteria_top20_gen))))
cat("Ranging from: ", min(colSums(otu_table(bacteria_top20_gen))), "to", 
    max(colSums(otu_table(bacteria_top20_gen))))

# Preparing data frame for plotting
top_genera_OTU <- as.data.frame(otu_table(bacteria_top20_gen))
top_genera_tax <- as.data.frame(tax_table(bacteria_top20_gen))
...
ggplot(tidy_genera, aes(x = reorder(Sample, Abundance), y = Abundance, fill = reorder(genera, Abundance))) +
  ...
  xlab("Sample") +
  ylab("Relative abundance") +
  ggtitle("Bacteria: Top 20 Genera") +
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()) +
  theme(legend.position = "bottom", legend.title = element_blank()) + 
  theme(legend.key.height = unit(0.3, "cm"), legend.key.width = unit(0.3, "cm")) + 
  theme(legend.text = element_text(size = 7)) +
...
ggsave(paste0(path_plots, "/Bacteria_top_genera.png"))


##### [6] Defining core #####

### Defining core bacterial species (>0.0001 relative abundance and present in 80% of all samples)
## Tax glom at species level
bacteria_spe = speedyseq::tax_glom(bacteria_decontam, taxrank = "Species")

saveRDS(bacteria_spe, paste0(pathout, "/bacteria_species.rds"))

## Transforming data into relative abundance and setting parameters for filtering
bacteria_spe_rel <- microbiome::transform(bacteria_spe, "compositional")
prev <- 0.8
abund <- (1/ntaxa(bacteria_spe)) #* 2)

# Getting mean species abundance
total_abundance <- rowSums(otu_table(bacteria_spe_rel), na.rm = TRUE)
...
bacteria_core <- core(bacteria_spe_rel, detection = abund, prevalence = prev, include.lowest = T)

## Getting species names and ids for heatmap
taxonomy_core <- as.data.frame(tax_table(bacteria_core))
...
sample_core <- as.data.frame(sample_data(bacteria_core))

## Preparing matrix for heatmap: Creating untransformed phyloseq with core species
bacteria_core_prune = prune_taxa(core_id, bacteria_spe)

# Saving untransformed data of core species
saveRDS(bacteria_core_prune, paste0(pathout, "/bacteria_core.rds"))

## Creating log-transformed matrix for heatmap
bacteria_core_log = microbiome::transform(bacteria_core_prune, "log10")
core_matrix <- data.matrix(as.data.frame(otu_table(bacteria_core_log)))
...
## Saving heatmap
png(file=paste0(path_plots, "/Bacteria_Heatmap.png"),
...
core_tax <- as.data.frame(tax_table(bacteria_core_prune))

write.csv(core_tax, paste0(pathout, "/Bacteria_core.csv"), row.names = T)
